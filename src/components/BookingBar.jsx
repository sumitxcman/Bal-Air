...
export default function BookingBar({ onSearch }) {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto bg-white/95 backdrop-blur-md border border-[#C5A028]/40 rounded-2xl p-4 sm:p-6 shadow-xl relative z-20">
      
      <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
        
        {/* Check-In */}
        <div>
          <label className="text-[11px] font-bold uppercase tracking-widest text-[#C5A028] block mb-1.5 flex items-center gap-1">
      ...
      <div className="mt-3 pt-3 border-t border-gray-200 flex flex-wrap items-center justify-between text-[11px] text-gray-500">
        <span className="flex items-center gap-1 text-[#C5A028] font-bold">
          <Sparkles className="w-3 h-3 text-[#C5A028]" /> Official Website Best Rate Guarantee (₹2,500/night vs Agoda ₹2,690)
        </span>
        <span className="hidden sm:inline text-gray-600 font-medium">Check-in: 2:00 PM | Check-out: 12:00 PM</span>
      </div>
