...
export default function HomePage({ setActiveTab, onSelectRoom, onAddToCart, onOpenGiftModal }) {
  ...
  return (
    <div className="space-y-12 sm:space-y-20 pb-16">
      ...
      {/* Hero Section */}
      <section className="relative min-h-[85vh] sm:min-h-[90vh] flex flex-col justify-between pt-10 sm:pt-16 pb-10 px-3 sm:px-6 lg:px-8 overflow-hidden">
        ...
        <div className="absolute inset-0 z-0">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-t from-[#F9F8F5] via-white/80 to-white/30"></div>
        </div>

        {/* Hero Main Content */}
        <div className="relative z-10 max-w-5xl mx-auto text-center space-y-4 sm:space-y-6 pt-6 sm:pt-12">
          
          {/* Badge */}
          <div className="inline-flex items-center gap-1.5 bg-white/90 border border-[#C5A028]/40 px-3 sm:px-4 py-1.5 rounded-full text-[10px] sm:text-xs text-[#C5A028] font-bold uppercase tracking-widest backdrop-blur-md shadow-md">
            <Sparkles className="w-3 sm:w-3.5 h-3 sm:h-3.5 shrink-0" />
            <span>5-Star Boutique Hospitality • Dehradun</span>
          </div>

          {/* Main Title */}
          <h1 className="text-3xl sm:text-6xl lg:text-7xl font-serif font-extrabold tracking-tight text-[#0B0B0C] leading-tight">
            BEL AIR <span className="gold-gradient-text block mt-0.5 sm:mt-1">LUXURY STAY</span>
          </h1>

          <p className="text-sm sm:text-xl font-serif italic text-[#0B0B0C] font-semibold max-w-2xl mx-auto px-2">
            "{HOTEL_DETAILS.tagline}"
          </p>

          {/* Action CTAs */}
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2 sm:pt-4 max-w-xs sm:max-w-none mx-auto">
            <button
              onClick={() => setActiveTab('rooms')}
              className="gold-button w-full sm:w-auto px-8 py-3.5 rounded-lg text-xs font-bold uppercase tracking-wider shadow-xl flex items-center justify-center gap-2"
            >
              Explore Suites
            ...
            <button
              ...
              className="gold-outline-button w-full sm:w-auto px-8 py-3.5 rounded-lg text-xs font-bold uppercase tracking-wider backdrop-blur-md shadow-xs"
            >
              View Packages
            </button>
          </div>
      ...
      <section className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="bg-white border border-[#C5A028]/30 rounded-2xl overflow-hidden shadow-xl grid grid-cols-1 lg:grid-cols-12 gap-0 items-center">
          ...
          <div className="lg:col-span-5 relative aspect-[4/3] lg:aspect-auto lg:h-full">
            ...
            <span className="absolute top-3 left-3 bg-white/90 text-[#C5A028] border border-[#C5A028]/40 text-[9px] sm:text-[10px] uppercase font-bold tracking-widest px-2.5 py-1 rounded shadow-xs">
              24/7 Royal Concierge Desk
            </span>
          </div>

          <div className="lg:col-span-7 p-4 sm:p-8 space-y-4">
            <div className="flex items-center gap-1.5 text-[#C5A028]">
              <Crown className="w-4.5 h-4.5" />
              <span className="text-[11px] sm:text-xs uppercase tracking-widest font-bold">Grand Lobby & Welcome Desk</span>
            </div>

            <h2 className="text-2xl sm:text-3xl font-serif font-bold text-[#0B0B0C] leading-tight">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs text-[#0B0B0C] pt-2 border-t border-gray-200">
              <div>
                <span className="text-[10px] text-gray-500 block uppercase font-medium">Address Location</span>
                <strong className="text-[#0B0B0C] text-[11px] block">{HOTEL_DETAILS.address}</strong>
              </div>
              <div>
            ...
            <div className="pt-2 flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="bg-emerald-600 hover:bg-emerald-500 text-white px-5 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-2 shadow-md"
              >
              ...
              <button
                ...
                className="gold-outline-button px-5 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider text-center"
              >
                View Directions & Map
              </button>
            </div>
      ...
      <section className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 space-y-8 sm:space-y-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3 border-b border-gray-200 pb-4 sm:pb-6">
          <div>
            <span className="text-[10px] sm:text-xs font-bold uppercase tracking-widest text-[#C5A028] block mb-0.5 sm:mb-1">
          ...
          <button
            onClick={() => setActiveTab('rooms')}
            className="text-xs font-bold text-[#C5A028] hover:underline flex items-center gap-1 uppercase tracking-widest self-start sm:self-auto"
          >
      ...
      <section className="bg-[#F5F2EA] py-12 sm:py-16 border-y border-[#C5A028]/20 relative">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 space-y-8 sm:space-y-12">
            ...
            {featuredPackages.map((pkg) => (
              <div key={pkg.id} className="bg-white border border-gray-200 hover:border-[#C5A028] rounded-xl overflow-hidden group transition-all duration-300 flex flex-col justify-between shadow-md hover:shadow-xl">
                ...
                <div className="p-5 sm:p-6 flex-1 flex flex-col justify-between space-y-4">
                  <div>
                    <h3 className="text-base sm:text-lg font-serif font-bold text-[#0B0B0C] group-hover:text-[#C5A028] transition-colors mb-2">
                      {pkg.name}
                    </h3>
                    <p className="text-xs text-[#55534E] mb-3">{pkg.description}</p>
                    
                    <ul className="space-y-1 text-xs text-[#0B0B0C] mb-4">
                      {pkg.highlights.map((h, i) => (
                        <li key={i} className="flex items-center gap-1.5 text-[11px] font-medium">
                          <CheckCircle2 className="w-3.5 h-3.5 text-[#C5A028] shrink-0" />
                          <span>{h}</span>
                        </li>
                      ))}
                    </ul>
                  ...
                  <div className="pt-3 border-t border-gray-200 flex items-center justify-between">
                    <div>
                      ...
                      <span className="text-base sm:text-lg font-serif font-bold text-[#C5A028]">₹{pkg.price.toLocaleString('en-IN')}</span>
                    </div>
          ...
          <div className="text-center pt-2">
            <button
              onClick={() => setActiveTab('packages')}
              className="gold-outline-button w-full sm:w-auto px-8 py-3 rounded-lg text-xs font-bold uppercase tracking-widest shadow-xs"
            >
      ...
      <section className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
          ...
          <div className="relative rounded-2xl overflow-hidden min-h-[340px] sm:min-h-[380px] p-6 sm:p-8 flex flex-col justify-between border border-gray-200 shadow-xl group">
            <img 
            ...
            <div className="relative z-10">
              <span className="text-[10px] sm:text-xs uppercase tracking-widest text-[#F3D779] font-bold flex items-center gap-1 drop-shadow-xs">
            ...
            <div className="relative z-10 space-y-3">
              ...
              <button
                ...
                className="gold-button w-full sm:w-auto px-5 py-2.5 rounded text-xs font-bold uppercase tracking-wider text-center"
              >
      ...
      {/* Gift Cards Banner */}
      <section className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="bg-gradient-to-r from-white via-[#F5F2EA] to-white border border-[#C5A028]/50 rounded-2xl p-6 sm:p-12 flex flex-col md:flex-row items-center justify-between gap-6 sm:gap-8 shadow-xl relative overflow-hidden">
          
          <div className="space-y-2 sm:space-y-3 max-w-xl text-center md:text-left">
            <span className="text-[10px] sm:text-xs uppercase tracking-widest text-[#C5A028] font-bold flex items-center justify-center md:justify-start gap-1">
          ...
          <button
            ...
            className="gold-button w-full md:w-auto px-8 py-3.5 sm:py-4 rounded-xl text-xs font-extrabold uppercase tracking-widest shadow-xl shrink-0"
          >
      ...
      <section className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 space-y-6 sm:space-y-8">
        ...
        <div className="text-center max-w-xl mx-auto space-y-1.5 sm:space-y-2">
          <span className="text-[10px] sm:text-xs uppercase tracking-widest text-[#C5A028] font-bold">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {GOOGLE_REVIEWS.map((rev) => (
            <div key={rev.id} className="bg-white border border-gray-200 p-5 rounded-xl space-y-3 flex flex-col justify-between shadow-sm hover:shadow-md transition-shadow">
              <div>
              ...
              <div className="pt-2.5 border-t border-gray-200 flex items-center gap-3">
                <div className="w-7 h-7 rounded-full bg-[#C5A028] text-white font-bold text-xs flex items-center justify-center shadow-xs shrink-0">
                  {rev.avatar}
                </div>
                <div>
                  <h4 className="text-xs font-bold text-[#0B0B0C]">{rev.author}</h4>
                  <span className="text-[10px] text-emerald-700 font-bold block">✓ Verified ({rev.room})</span>
                </div>
              </div>
            </div>
          ))}
